...
class AuthorsRegisterTest(AuthorsBaseTest):
    ...
    def test_passwords_do_not_match(self):
        ...
        def callback(form):
            ...
            password_confirmation_field = self.get_by_placeholder(form,'Repeat your password')
            password_field.send_keys('P@ssw0rd')
            password_confirmation_field.send_keys('P@ssw0rd_Different')
            password_confirmation_field.send_keys(Keys.ENTER)
            form = self.get_form()
            self.assertIn('Password and password_confirmation must be equal', form.text)
