...
def generate_images(prompt,negative_prompt,num_images_per_prompt,num_inference_steps,height, width, seed, guidance_scale):
    ...
    images = pipeline(prompt=prompt, num_images_per_prompt=num_images_per_prompt,negative_prompt=negative_prompt,num_inference_steps=num_inference_steps
                    ,height=height,width=width, generator=generator)['images']
...
with st.sidebar:
    ...
    num_inference_steps = st.number_input("Número de passos inferencias", min_value=1,max_value=100,value=50)
    height = st.selectbox("Altura da image",[256,512,768,1024],index=1)
    width = st.selectbox("Largura da image",[256,512,768,1024],index=1)
    seed = st.number_input("Seed", min_value=0, max_value=99999, value=42)
    guidance_scale = st.number_input("Escala de orientação", min_value=0.0, max_value=20.0, value=7.5, step=0.1)
